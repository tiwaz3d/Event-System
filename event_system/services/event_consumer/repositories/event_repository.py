...
class EventRepository:
    ...
    async def get_event_counts(self) -> Dict[str, int]:
        result = await self._session.execute(select(Event.event_type, func.count(Event.id)).group_by(Event.event_type))
        return dict(result.all())

    async def get_recent_events(self, limit: int = 20) -> List[Event]:
        result = await self._session.execute(select(Event).order_by(Event.created_at.desc()).limit(limit))
